...
const kpis: Kpi[] = [
  { label: "Event Severity", value: "Medium", foot: "ตรวจพบการแก้ไขข้อมูลทางคลินิก", icon: "!", target: "risk" },
  ...
  { label: "AI Involvement", value: "AI Suggested", foot: "ผ่านการตรวจสอบและปรับแก้โดยแพทย์", icon: "✦", target: "ai-trace" },
  { label: "Policy Checks", value: "5 Passed", foot: "1 warning · 0 failed", icon: "✓", target: "risk" },
];
            ...
            <section className="ad-audit-head">
              <div className="ad-head-row">
                ...
                <div className="ad-head-actions">
                  ...
                  <button className="ad-btn" type="button" onClick={() => setModal("actions")}>More Actions</button>
                  <button className="ad-btn ad-primary" type="button" onClick={() => setModal("export")}>⇩ Export Evidence</button>
                </div>
              </div>
            ...
            <section className="ad-kpis" aria-label="Audit detail KPIs">
              {kpis.map((kpi) => (
                <article className="ad-kpi" key={kpi.label} onClick={() => { window.location.hash = kpi.target; }}>
                  ...
                  {kpi.foot ? <div className="ad-kpi-foot">{kpi.foot}</div> : null}
                </article>
              ))}
            </section>
